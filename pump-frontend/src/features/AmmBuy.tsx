...
const AmmBuy: React.FC = () => {
  ...
  const handleBuy = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!poolAddress || !amount || !maxQuoteAmount) {
      ...
      // Base token (the token you want to buy)
      ...
      // Quote token (usually SOL or USDC that you're paying with)
      ...
      const tx = await pumpAmmProgram.methods
        .buy(
          new BN(amount), // baseAmountOut - amount of tokens to buy
          new BN(maxQuoteAmount) // maxQuoteAmountIn - max amount willing to pay
        )
        .accounts({
          pool: poolPubkey,
          user: publicKey,
  ...
  return (
    <div>
      ...
      {!publicKey ? (
        <div>Connect your wallet to buy tokens.</div>
      ) : (
        <form onSubmit={handleBuy}>
          ...
          <div>
            ...
            <input
              ...
              placeholder="Enter maximum amount you're willing to pay"
              disabled={loading}
              required
            />
          </div>
          ...
          <button type="submit" disabled={loading || !poolAddress || !amount || !maxQuoteAmount}>
