...
const Deposit: React.FC = () => {
  ...
  const handleDeposit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1. Build deposit instructions (using a placeholder method, adjust as needed for your SDK)
      // You may need to fetch the pool object and use the correct method for your SDK version
      // This is a placeholder for the real deposit call
      const instructions = await pumpAmmSdk.depositInstructions(
        poolMintKey,
        new BN(amount),
        parseFloat(slippage),
        publicKey
      ...
      setStatus(`Deposit transaction sent! Tx: ${txid}`);
    } catch (err: any) {
  ...
  return (
    <div>
      ...
      <form onSubmit={handleDeposit}>
        ...
        <button type="submit" disabled={!publicKey || loading || !poolMint || !amount}>Deposit</button>
      </form>
      {loading && <div>Sending transaction...</div>}
      {status && <div>{status}</div>}
      {!publicKey && <div style={{ color: 'orange' }}>Connect your wallet to deposit.</div>}
    </div>
  );
};
